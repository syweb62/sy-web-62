...
import { Trash2, ShoppingBag, ArrowRight, User, Phone, MapPin, RefreshCw, Check } from "lucide-react"
...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    if (savedCustomerData) {
      ...
      console.log("Customer data auto-filled from previous order")
    }
  }, [])
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Generate order ID
      const newOrderId = `ORD-${Date.now().toString().slice(-6)}-${Math.random().toString(36).substr(2, 4).toUpperCase()}`
      ...
      const orderData = {
        orderId: newOrderId,
        customer: {
          name: validation.sanitizeInput(formData.name),
          phone: formData.phone.replace(/\D/g, ""),
          address: validation.sanitizeInput(formData.address),
          notes: formData.notes ? validation.sanitizeInput(formData.notes) : undefined,
        },
        items: cartItems,
        ...
        discount: {
          ...
          percentage: discountApplied ? 15 : 0,
        },
        totals: {
          subtotal: subtotal,
          ...
          vat: vatAmount,
          delivery: isFreeDelivery ? 0 : deliveryAmount,
          total: isFreeDelivery ? finalTotal - deliveryFee : finalTotal,
        },
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-2xl mx-auto text-center">
              ...
              <p className="text-gray-300 mb-4">
                ...
                <span className="text-gold">{formData.phone}</span> to confirm the details and delivery time.
              </p>
              <div className="bg-gold/10 border border-gold/20 rounded-lg p-4 mb-8">
                <p className="text-sm text-gray-300">
                  ✨ Your information has been saved for faster checkout on future orders!
                </p>
              </div>
